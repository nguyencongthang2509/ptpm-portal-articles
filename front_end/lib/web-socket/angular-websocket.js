(function (global, factory) {
  ...
  function $WebSocketProvider($rootScope, $q, $timeout, $websocketBackend) {
    function $WebSocket(url, protocols, options) {
      ...
      this.reconnectIfNotNormalClose =
        (options && options.reconnectIfNotNormalClose) || false;
    ...
    $WebSocket.prototype._connect = function _connect(force) {
      if (
        ...
        this.socket.onmessage = _angular2.default.bind(
          this,
          this._onMessageHandler
        );
        this.socket.onopen = _angular2.default.bind(this, this._onOpenHandler);
        this.socket.onerror = _angular2.default.bind(
          this,
          this._onErrorHandler
        );
        this.socket.onclose = _angular2.default.bind(
          this,
    ...
    $WebSocket.prototype.fireQueue = function fireQueue() {
      while (
        ...
        this.socket.send(
          isString(data.message) || this.binaryType != "blob"
            ? data.message
    ...
    $WebSocket.prototype.notifyCloseCallbacks = function notifyCloseCallbacks(
      event
    ) {
    ...
    $WebSocket.prototype.notifyErrorCallbacks = function notifyErrorCallbacks(
      event
    ) {
    ...
    $WebSocket.prototype._onCloseHandler = function _onCloseHandler(event) {
      ...
      if (
        (this.reconnectIfNotNormalClose &&
          event.code !== this._normalCloseCode) ||
    ...
    $WebSocket.prototype._onMessageHandler = function _onMessageHandler(
      message
    ) {
      var pattern;
      var self = this;
  ...
  function $WebSocketBackendProvider($log) {
    ...
    this.createWebSocketBackend = function createWebSocketBackend(
      url,
      protocols
    ) {
